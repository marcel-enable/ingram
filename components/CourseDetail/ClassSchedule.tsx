...
const formatPrice = (price: number | null) => (price ? price : 0);

const formatType = (type: string | null) => {
    if (type === 'inPersonEvent') return 'ILT';
    else if (type === 'webinar') return 'VILT';
    else return '';
};
...
const ClassSchedule = ({
    ...
    const { data, loading, error } = useQuery(query, {
        variables: { slug: routeParams.courseSlug },
    });
    ...
    useEffect(() => {
        if (data) {
            ...
            setType(formatType(data.CourseGroupBySlug?.kind));
        } else if (error) {
